...
export default function HomeScrollAnimations() {
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const portfolioSection = document.querySelector<HTMLElement>("[data-home-portfolio]");
      const portfolioShell = portfolioSection?.querySelector<HTMLElement>("[data-portfolio-shell]");
      const portfolioHead = portfolioSection?.querySelector<HTMLElement>("[data-portfolio-head]");
      const portfolioFilters = portfolioSection?.querySelector<HTMLElement>("[data-portfolio-filters]");
      const portfolioCards = portfolioSection?.querySelector<HTMLElement>("[data-portfolio-cards]");
      const portfolioCardItems = Array.from(portfolioSection?.querySelectorAll<HTMLElement>("[data-portfolio-card]") ?? [])
        .filter((node) => node.offsetParent !== null);
      const portfolioCardImages = portfolioCardItems
        .map((card) => card.querySelector<HTMLElement>("img"))
        .filter((image): image is HTMLElement => Boolean(image));

      if (portfolioSection && portfolioShell) {
        gsap.fromTo(
          portfolioShell,
          { y: 68 },
          {
      ...
      if (portfolioSection && portfolioHead) {
        gsap.fromTo(
          portfolioHead,
          { autoAlpha: 0.35, y: 46, scale: 0.985 },
          {
            autoAlpha: 1,
            y: 0,
            scale: 1,
      ...
      if (portfolioSection && portfolioFilters) {
        gsap.fromTo(
          portfolioFilters,
          { autoAlpha: 0.4, y: 26, scale: 0.96 },
          {
            autoAlpha: 1,
            y: 0,
            scale: 1,
      ...
      if (portfolioSection && portfolioCards) {
        gsap.fromTo(
          portfolioCards,
          { y: 54, autoAlpha: 0.72 },
          {
      ...
      if (portfolioCardItems.length) {
        gsap.from(portfolioCardItems, {
          autoAlpha: 0,
          y: 42,
          scale: 0.97,
          duration: 0.75,
      ...
      portfolioCardItems.forEach((card, index) => {
        const driftY = index % 3 === 1 ? -18 : index % 3 === 2 ? 18 : -10;
      ...
      portfolioCardImages.forEach((image, index) => {
        const startY = index % 2 === 0 ? -5 : 5;
        const endY = index % 2 === 0 ? 5 : -5;

        gsap.fromTo(
          image,
          { scale: 1.08, yPercent: startY },
          {
            scale: 1.02,
            yPercent: endY,
            ease: "none",
