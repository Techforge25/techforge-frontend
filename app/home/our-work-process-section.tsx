...
const timelineLineClass =
  "pointer-events-none absolute left-1/2 top-[152px] h-[2px] w-[1280px] -translate-x-1/2 bg-[linear-gradient(90deg,rgba(80,95,251,0)_0%,#505FFB_3.6%,#7F8AFF_21.7%,#505FFB_38.4%,#505FFB_65.4%,#7F8AFF_83.1%,#505FFB_96%,rgba(80,95,251,0)_100%)]";
...
function TimelineStep({ step }: { step: (typeof processSteps)[number] }) {
  return (
    <div data-process-step className="absolute inset-y-0" style={{ left: step.left }}>
      ...
      <div className={`absolute left-1/2 w-[168px] xl:w-[184px] ${step.textClassName}`}>
        <p className="font-normal text-[20px] leading-[1.05] text-white xl:text-[22px]">{step.title}</p>
        <p className="mt-2 text-[13px] leading-[1.5] text-[#cac6dd] xl:text-[14px] xl:leading-[1.55]">
          {step.description}
        </p>
      </div>
    </div>
...
function TimelineTrack() {
  return (
    <div className="relative h-[320px] w-full max-w-[1380px]">
...
function ProcessTimelineMobile() {
  return (
    <div className="relative block h-[860px] w-full overflow-hidden lg:hidden">
      <div className="absolute left-1/2 top-1/2 h-[320px] w-[1280px] -translate-x-1/2 -translate-y-1/2 rotate-[90deg] scale-[0.68]">
        <TimelineTrack />
      </div>
    </div>
  );
  ...
  return (
    <section data-home-process className="relative overflow-hidden bg-[#00000c]">
      ...
      <SectionPaddingX120R72>
        <SectionPaddingY72>
          <div className="relative z-10 mx-auto flex w-full max-w-[1380px] flex-col items-center gap-8 sm:gap-10 lg:gap-[72px]">
